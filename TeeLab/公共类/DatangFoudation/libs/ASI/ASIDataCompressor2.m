...
- (NSError *)setupStream
{
	...
	zStream.avail_in = 0;
	zStream.next_in = 0;
    //deflateInit(&zStream, COMPRESSION_AMOUNT);
	int status =deflateInit(&zStream, COMPRESSION_AMOUNT);// deflateInit2(&zStream, COMPRESSION_AMOUNT, Z_DEFLATED, (15+16), 8, Z_DEFAULT_STRATEGY);
	if (status != Z_OK) {
